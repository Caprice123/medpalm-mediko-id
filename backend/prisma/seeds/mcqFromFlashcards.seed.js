...
// Same topic/subtopic structure as flashcardNodes.seed.js — slugs must match.
// Cards are re-declared here so this script is self-contained.
const SEED_DATA = [
  {
...
// Pick 3 distractors from other cards in the same subtopic.
// Falls back to repeating from the pool if there are fewer than 3 other cards.
function pickDistractors(allBacks, correctBack) {
  const pool = allBacks.filter(b => b !== correctBack)
...
async function seedMcqFromFlashcards() {
  console.log('Seeding MCQ questions from flashcard data...')

  for (const topic of SEED_DATA) {
    const topicNode = await prisma.feature_nodes.findUnique({ where: { slug: topic.slug } })
    if (!topicNode) {
      console.log(`⚠ Topic node not found: ${topic.slug} — run flashcardNodes.seed.js first`)
      continue
    }
    console.log(`✓ Topic: ${topicNode.name}`)

    for (const sub of topic.subtopics) {
      const subNode = await prisma.feature_nodes.findUnique({ where: { slug: sub.slug } })
      if (!subNode) {
        console.log(`  ⚠ Subtopic node not found: ${sub.slug}`)
        continue
      }

      const existing = await prisma.feature_node_records.count({
        where: { node_id: subNode.id, record_type: 'mcq_question' },
      })
      if (existing > 0) {
        console.log(`  ⊘ Skipped ${subNode.name} — ${existing} MCQ questions already linked`)
        continue
      }

      const allBacks = sub.cards.map(c => c.back)
      let count = 0

      for (const card of sub.cards) {
        const distractors = pickDistractors(allBacks, card.back)

        // Build shuffled options with the correct answer at a random position
        const options = shuffle([card.back, ...distractors])
        const correctIndex = options.indexOf(card.back)

        const question = await prisma.mcq_questions.create({
          data: {
            question: card.front,
            options,
        ...
        await prisma.feature_node_records.create({
          data: { node_id: subNode.id, record_type: 'mcq_question', record_id: question.id },
        })
      ...
      await prisma.node_statistics.upsert({
        where: { node_id_record_type: { node_id: subNode.id, record_type: 'mcq_question' } },
        create: { node_id: subNode.id, record_type: 'mcq_question', total_count: count },
        update: { total_count: count },
      })

      console.log(`  ✓ ${subNode.name}: ${count} MCQ questions created`)
    }
  }

  console.log('\nMCQ from flashcards seeding completed!')
}

seedMcqFromFlashcards()
  .then(async () => {
