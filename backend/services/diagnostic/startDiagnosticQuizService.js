...
export class StartDiagnosticQuizService extends BaseService {
  static async call({ quizId, userId }) {
    const result = await prisma.$transaction(async tx => {
      ...
      if (!hasSubscription) {
        throw new ValidationError('Active subscription required to access diagnostic quizzes')
      }
      ...
      if (!quiz.diagnostic_questions || quiz.diagnostic_questions.length === 0) {
      ...
      const questionIds = quiz.diagnostic_questions.map(q => q.id)
      const userProgress = await tx.user_diagnostic_progress.findMany({
        where: {
      ...
      const questionsWithWeights = quiz.diagnostic_questions.map(question => {
      ...
      const attempt = await tx.diagnostic_quiz_attempts.create({
        data: {
          user_id: parseInt(userId),
          quiz_id: quiz.id,
