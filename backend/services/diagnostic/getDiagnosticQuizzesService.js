...
export class GetDiagnosticQuizzesService extends BaseService {
  static async call(filters = {}) {
    ...
    const quizzes = await prisma.diagnostic_quizzes.findMany({
      where,
      take,
      skip,
      include: {
    ...
    const diagnosticConstant = await GetConstantsService.call(['diagnostic_quiz_cost'])
    const cost = diagnosticConstant.diagnostic_quiz_cost
