...
export class SubmitDiagnosticAnswersService extends BaseService {
  static async call({ attemptId, userId, answers }) {
    const result = await prisma.$transaction(async tx => {
      // Get the attempt
      const attempt = await tx.diagnostic_quiz_attempts.findUnique({
      ...
      const questionIds = attempt.diagnostic_quiz.diagnostic_questions.map(q => q.id)
      ...
      const questionMap = {}
      attempt.diagnostic_quiz.diagnostic_questions.forEach(q => {
        questionMap[q.id] = q
      })
      ...
      for (const answer of answers) {
        ...
        await tx.diagnostic_quiz_answers.create({
          data: {
        ...
        // Update user_diagnostic_progress for spaced repetition
        await tx.user_diagnostic_progress.upsert({
          where: {
            user_id_question_id: {
          ...
          create: {
            user_id: parseInt(userId),
            question_id: answer.questionId,
            quiz_id: attempt.diagnostic_quiz.id,
      ...
      // Update attempt
      await tx.diagnostic_quiz_attempts.update({
        where: { id: attempt.id },
        data: {
          status: 'completed',
      ...
      return {
        ...
        score: score,
        image_url: attempt.diagnostic_quiz.image_url,
