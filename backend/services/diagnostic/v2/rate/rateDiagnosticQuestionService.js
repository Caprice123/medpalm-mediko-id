...
export class RateDiagnosticQuestionService extends BaseService {
  static async call({ userId, recordId, rating }) {
    ...
    const reviewStateOp = prisma.user_review_states.upsert({
      ...
      create: {
        user_id: userId, record_type: RECORD_TYPE, record_id: recordId,
        interval, ease_factor: easeFactor, due_date: dueDate, review_count: 1, last_rating: rating,
      },
      update: {
        interval, ease_factor: easeFactor, due_date: dueDate,
        review_count: { increment: 1 }, last_rating: rating, updated_at: new Date(),
      },
    })
    ...
    if (!topicNodeId) {
      await prisma.$transaction([reviewStateOp, learnedItemOp])
      return
    }
