...
export class UpdateDiagnosticQuizService extends BaseService {
  static async call({
    quizId,
    title,
    description,
    ...
    const existingQuiz = await prisma.diagnostic_quizzes.findUnique({
    ...
    const updatedQuiz = await prisma.$transaction(async tx => {
      ...
      const quiz = await tx.diagnostic_quizzes.update({
        where: { unique_id: quizId },
        data: {
          title,
