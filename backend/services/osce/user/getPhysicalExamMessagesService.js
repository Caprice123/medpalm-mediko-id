...
export class GetPhysicalExamMessagesService extends BaseService {
  static async call(userId, sessionId, options = {}) {
    ...
    // Get physical exam interactions for this session with pagination
    // Order DESC to get latest first, then reverse in controller
    const messages = await prisma.osce_session_physical_interactions.findMany({
      where,
      orderBy: {
