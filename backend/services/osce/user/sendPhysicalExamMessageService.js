...
export class SendPhysicalExamMessageService extends BaseService {
  static async call({
    ...
    try {
      ...
      let systemPrompt = constantsMap.osce_practice_physical_exam_prompt
      let knowledgeBase = ""
      ...
      console.log(session.osce_session_physical_interactions)

      // Get AI response with streaming
      const result = await this._getAIStreamingResponse({
        model: session.osce_topic.ai_model,
        systemPrompt,
        conversationHistory: session.osce_session_physical_interactions,
        userMessage: message,
      ...
      console.error('[SendPhysicalExamMessageService] Error:', error)
      if (onError) {
        onError(error)
      }
      throw error
  ...
  static async _getAIStreamingResponse({
    ...
    try {
      ...
      console.log(`Physical exam stream ${streamAborted ? 'aborted' : 'completed'}.`)
      ...
      // Save messages to database (physical interactions table)
      try {
        userMessageRecord = await prisma.osce_session_physical_interactions.create({
          data: {
        ...
        aiMessageRecord = await prisma.osce_session_physical_interactions.create({
          data: {
            osce_session_id: sessionId,
            sender_type: 'ai',
