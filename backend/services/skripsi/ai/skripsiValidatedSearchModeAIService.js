...
export class SkripsiValidatedSearchModeAIService extends BaseService {
  /**
   ...
   */
  static async call({ userId, tabId, message }) {
      // Get skripsi constants for configuration
        ...
        where: {
          key: {
            in: [
              ...
              'skripsi_validated_last_message_count',
              'skripsi_validated_system_prompt',
              'skripsi_validated_search_count',
              'skripsi_validated_threshold',
              'skripsi_validated_rewrite_enabled',
              'skripsi_validated_rewrite_prompt',
            ]
          }
        }
      })
      ...
      const modeActive = constantsMap.skripsi_validated_enabled === 'true'
      if (!modeActive) {
          throw new ValidationError("Validated mode sedang tidak aktif. Silakan pilih mode lain")
      }

      const modelName = constantsMap.skripsi_validated_model || 'gemini-2.5-flash'
      const embeddingModel = constantsMap.skripsi_validated_embedding_model || 'gemini-2.5-flash'
      const creditsPerMessage = parseFloat(constantsMap.skripsi_validated_cost) || 10
      const lastMessageCount = parseInt(constantsMap.skripsi_validated_last_message_count) || 10
      const systemPrompt = constantsMap.skripsi_validated_system_prompt
      const searchCount = parseInt(constantsMap.skripsi_validated_search_count) || 5
      const threshold = parseFloat(constantsMap.skripsi_validated_threshold) || 0.3
      const rewriteEnabled = constantsMap.skripsi_validated_rewrite_enabled === 'true'
      const rewritePrompt = constantsMap.skripsi_validated_rewrite_prompt
      ...
      if (rewriteEnabled) {
        ...
        console.log(tabId, lastMessageCount)

        // Get recent conversation history for context (reuse lastMessageCount)
        const recentMessages = await prisma.skripsi_messages.findMany({
          where: {
            tab_id: tabId,
          },
          orderBy: { id: 'desc' },
