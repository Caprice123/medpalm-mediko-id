...
function SubtopicDropdown({ options, selected, onChange, isLoading }) {
  ...
  const handleToggleOpen = () => {
    if (!open && triggerRef.current) {
      ...
      setPanelStyle(window.innerHeight - rect.bottom < 300
  ...
  const triggerLabel = selected.length === 0 ? 'Semua subtopik' : `${selected.length} subtopik dipilih`
  ...
  const panel = open && !isLoading && (
    <DropdownPanel ref={panelRef} style={panelStyle}>
      <DropdownHeader>
        ...
        <DropdownActions>
          <DropdownAction type="button" onClick={() => onChange(options.map(o => o.value))}>Pilih semua</DropdownAction>
    ...
    <Wrap>
      ...
      <Row>
        <Col style={{ flex: 1, minWidth: 220 }}>
          <FieldLabel>Subtopik ({subtopics.length} tersedia)</FieldLabel>
          <SubtopicDropdown
            options={options}
            selected={selected}
        ...
        <Col>
          ...
          <PresetsRow>
            {PRESETS.map(p => (
              <PresetBtn key={p} $active={!isCustom && preset === p} onClick={() => { setPreset(p); setIsCustom(false) }}>
                {p}
              </PresetBtn>
            ))}
            <PresetBtn $active={isCustom} onClick={() => setIsCustom(true)}>Lainnya</PresetBtn>
            {isCustom && (
              <CountInput type="number" min={1} value={customVal} onChange={e => setCustomVal(e.target.value)} />
            )}
          </PresetsRow>
        </Col>

        <StartButton onClick={handleStart} disabled={isStarting || isLoadingSubtopics}>
