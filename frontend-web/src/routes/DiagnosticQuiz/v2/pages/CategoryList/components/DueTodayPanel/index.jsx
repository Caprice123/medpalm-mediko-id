...
import { PanelCard, EmptyText } from '../../CategoryList.shared.styles'
import {
...
export default function DueTodayPanel({ onStartAll, onStartNode, isStarting, batchSize }) {
  ...
  const { dueToday, loading } = useSelector(s => s.diagnosticNodes)
  ...
  return (
    <PanelCard>
      <HeaderRow>
        ...
        {dueToday && (
          <CountBlock style={{ marginLeft: 'auto' }}>
            ...
            <CountLabel>soal</CountLabel>
          </CountBlock>
        )}
        {subtopics.length > 0 && (
          <StartAllBtn onClick={onStartAll} disabled={isStarting}>
            {isStarting ? 'Memulai...' : `Mulai ${startCount} Soal`}
          </StartAllBtn>
        )}
      </HeaderRow>
      ...
      {subtopics.length > 0 ? (
        ...
        <EmptyText>Tidak ada soal yang perlu diulang hari ini.</EmptyText>
      )}
    </PanelCard>
  )
}
