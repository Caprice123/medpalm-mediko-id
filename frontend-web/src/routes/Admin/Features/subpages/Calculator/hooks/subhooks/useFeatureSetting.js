...
export const useFeatureSetting = (onClose) => {
  ...
  useEffect(() => {
    const onLoad = async () => {
      // Only fetch constants if user has 'calculator' permission
      if (!hasFeaturePermission('calculator')) {
        console.warn('User lacks permission to fetch calculator constants')
        return
      }
